...
const change = (id, value) => evaluate(`document.querySelector('[data-ref=${id}]').value=${JSON.stringify(value)}; document.querySelector('[data-ref=${id}]').dispatchEvent(new Event('change'))`);
...
try {
    ...
    await waitFor(`document.querySelectorAll('[data-ref=target-body] tr').length === 3`);
    // Leaflet is imported on mount, so the map can render after the tables.
    await waitFor(`document.querySelectorAll('.ais-station-icon').length === 3`);
    assert(await evaluate(`document.querySelectorAll('[data-ref=map] .leaflet-overlay-pane svg path').length > 2`), 'coverage polygons are drawn');
    assert(await evaluate(`document.querySelector('[data-ref=station-comparison]').textContent.includes('<img') && !document.querySelector('[data-ref=station-comparison] img')`), 'station name is plain text');
    assert(await evaluate(`document.querySelector('[data-ref=sim-clock]').textContent.includes('Paused')`));
    const clock = await evaluate(`document.querySelector('[data-ref=sim-clock]').textContent`);
    await click('[data-ref=fit-coverage]'); await capture('display-coverage');
    const aPaths = await evaluate(`document.querySelectorAll('[data-ref=map] .leaflet-overlay-pane svg path').length`);
    await change('coverage-channel', 'B');
    assert(await evaluate(`document.querySelectorAll('[data-ref=map] .leaflet-overlay-pane svg path').length < ${aPaths}`), 'disabled channel B contributes no contours');
    await change('coverage-channel', 'A');
    await click('[data-ref=show-coverage]');
    assert.equal(await evaluate(`document.querySelectorAll('[data-ref=map] .leaflet-overlay-pane svg path').length`), 2, 'only targets remain');
    await click('[data-ref=show-coverage]'); await click('[data-ref=show-lost]');
    assert.equal(await evaluate(`document.querySelectorAll('[data-ref=target-body] tr').length`), 4);
    await click('[data-ref=show-lost]');
    await click('[data-ref=target-body] tr[data-key="111000003"] button');
    assert(await evaluate(`document.querySelector('[data-ref=detail-summary]').textContent.includes('unavailable')`));
    await click('[data-ref=target-body] tr[data-key="111000001"] button');
    assert.equal(await evaluate(`document.querySelectorAll('[data-ref=provenance-body] tr').length`), 2);
    assert(await evaluate(`document.querySelector('[data-ref=provenance-body]').textContent.includes('Older report')`));
    await capture('display-provenance');
    await click('[data-ref=provenance-body] tr[data-key="s2"] button');
    await waitFor(`document.querySelector('[data-ref=detail-summary]').textContent.includes('52.010000')`);
    assert.equal(await evaluate(`document.querySelectorAll('[data-ref=target-body] tr').length`), 2);
    await waitFor(`document.querySelector('[data-ref=history-status]').textContent.includes('Station history sample')`);
    await click('[data-ref=message-body] button');
    assert(await evaluate(`document.querySelector('[data-ref=message-summary]').textContent.includes('Reception-time config revision 1')`));
    await call('Browser.grantPermissions', { origin, permissions: ['clipboardReadWrite', 'clipboardSanitizedWrite'] });
    await click('[data-ref=copy-nmea]');
    await waitFor(`document.querySelector('[data-ref=copy-status]').textContent.startsWith('Copied exact NMEA')`);
    ...
    await click('[data-ref=all-stations]');
    await waitFor(`document.querySelectorAll('[data-ref=target-body] tr').length === 3`);
    await click('[data-ref=station-comparison] tr[data-key="s1"] button');
    await waitFor(`document.querySelector('[data-ref=detail-summary]').textContent.includes('Published reception model parameters')`);
    await capture('display-capabilities');
    await waitFor(`document.querySelector('[data-ref=history-status]').textContent.includes('Station history sample')`);
    await click('[data-ref=message-body] button');
    await evaluate(`fixture.edited = true`);
    await waitFor(`document.querySelector('[data-ref=detail-summary]').textContent.includes('Config revision 2; RF revision 2')`);
    assert(await evaluate(`document.querySelector('[data-ref=message-summary]').textContent.includes('config revision 1') && document.querySelector('[data-ref=message-summary]').textContent.includes('Coast <img')`), 'historical receiver configuration stays pinned');
    await evaluate(`fixture.historyMode = 'gap'`);
    await waitFor(`document.querySelector('[data-ref=history-status]').textContent.startsWith('History gap')`);
    assert.equal(await evaluate(`document.querySelectorAll('[data-ref=message-body] tr').length`), 200);
    assert(await evaluate(`fixture.requests.some(r => r.url.includes('after=9007199254740993'))`), 'large cursor retained exactly');
    await click('[data-ref=pause-history]');
    ...
    assert.equal(await evaluate(`document.querySelector('[data-ref=sim-clock]').textContent`), clock, 'paused virtual clock does not advance');
    await evaluate(`fixture.historyMode = 'tail'; fixture.failure = 503`);
    await waitFor(`document.querySelector('[data-ref=display-view]').classList.contains('ais-stale')`);
    assert.equal(await evaluate(`document.querySelectorAll('[data-ref=target-body] tr').length`), 2, 'network failure retains observations');
    assert(await evaluate(`document.querySelector('[data-ref=live-status]').textContent.includes('Entire view is stale')`));
    ...
    await waitFor(`document.querySelectorAll('[data-ref=station-comparison] tr').length === 2 && document.querySelector('[data-ref=all-stations]').getAttribute('aria-pressed') === 'true'`);
    assert(await evaluate(`document.querySelector('[data-ref=selection-notice]').textContent.includes('removed')`));
    await click('[data-ref=target-body] tr[data-key="111000001"] button');
    await evaluate(`fixture.expired = true`);
    await waitFor(`document.querySelector('[data-ref=detail-summary]').textContent.includes('expired')`);
    await evaluate(`fixture.expired = false; fixture.removed = false; fixture.edited = false; fixture.run = 'fixture-run-2'`);
    await waitFor(`document.querySelector('[data-ref=selection-notice]').textContent.includes('restarted')`);
    assert.equal(await evaluate(`document.querySelector('[data-ref=detail-summary]').textContent`), 'Select a station or target.');
    assert.equal(await evaluate(`document.querySelector('[data-ref=raw-nmea]').value`), '');
    // Deliberately ignore AbortSignal in the fixture to exercise generation checks.
    await evaluate(`fixture.delaySelection = true; document.querySelector('[data-ref=station-selection] [data-id="s1"] input').click()`);
    await waitFor(`fixture.waits.length === 1`);
    await click('[data-ref=all-stations]');
    await evaluate(`fixture.release()`);
    await waitFor(`!document.querySelector('[data-ref=display-view]').classList.contains('ais-pending')`);
    assert.equal(await evaluate(`document.querySelectorAll('[data-ref=target-body] tr').length`), 3, 'late selection response is ignored');
    ...
    await waitFor(`document.querySelector('[data-ref=history-status]').textContent.includes('Station history sample')`);
    assert(await evaluate(`document.querySelector('[data-ref=message-body]').textContent.includes('Northern receiver') && !document.querySelector('[data-ref=message-body]').textContent.includes('Coast')`), 'late history response cannot restore another station');
    await click('[data-ref=station-selection] [data-id="s1"] input');
    await waitFor(`!document.querySelector('[data-ref=display-view]').classList.contains('ais-pending')`);
    await click('[data-ref=station-selection] [data-id="s2"] input');
    await waitFor(`document.querySelector('[data-ref=target-counts]').textContent.includes('2 receiving stations')`);
    assert.equal(await evaluate(`document.querySelectorAll('[data-ref=target-body] tr').length`), 3, 'multiselection is a union');
    await evaluate(`fixture.run = 'fixture-run-3'`);
    await waitFor(`document.querySelector('[data-ref=all-stations]').getAttribute('aria-pressed') === 'true' && document.querySelectorAll('[data-ref=target-body] tr').length === 3`);
    assert.equal(await evaluate(`document.querySelector('[data-ref=history-station]').value`), '', 'restart clears explicitly selected station history');
    await evaluate(`document.querySelector('[data-ref=station-filter]').value='Northern';document.querySelector('[data-ref=station-filter]').dispatchEvent(new Event('input'));document.querySelector('[data-ref=station-filter]').focus()`);
    const beforeFocus = await evaluate(`fixture.count`);
    await waitFor(`fixture.count > ${beforeFocus}`);
    assert.equal(await evaluate(`document.activeElement.dataset.ref`), 'station-filter', 'poll preserves keyboard focus');
    assert.equal(await evaluate(`document.querySelectorAll('[data-ref=station-comparison] tr').length`), 1);
    await evaluate(`document.querySelector('[data-ref=station-filter]').value='';document.querySelector('[data-ref=station-filter]').dispatchEvent(new Event('input'))`);
    await call('Emulation.setDeviceMetricsOverride', { width: 390, height: 844, deviceScaleFactor: 1, mobile: true });
    assert(await evaluate(`document.querySelector('.ais-observation-details').getBoundingClientRect().top >= document.querySelector('[data-ref=map]').getBoundingClientRect().bottom`), 'mobile details below map');
    ...
    await waitFor(`!document.querySelector('[data-ref=empty-stations]').hidden`);
    assert(await evaluate(`document.querySelector('[data-ref=empty-stations] a').getAttribute('href') === '/manager'`));
    ...
    await call('Network.setBlockedURLs', { urls: ['*leaflet-src.esm.js*', '*tile.openstreetmap.org*'] });
    await call('Page.navigate', { url: origin + '/display' });
    await waitFor(`document.querySelectorAll('[data-ref=target-body] tr').length === 3`);
    assert(await evaluate(`document.querySelector('[data-ref=tile-status]').textContent.includes('Map library unavailable')`));
    await call('Network.setBlockedURLs', { urls: ['*tile.openstreetmap.org*'] });
    await call('Page.navigate', { url: origin + '/display' });
    await waitFor(`document.querySelector('[data-ref=tile-status]').textContent.includes('Map tiles unavailable')`);
    assert.equal(await evaluate(`document.querySelectorAll('[data-ref=target-body] tr').length`), 3);
