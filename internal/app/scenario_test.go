...
func newScenario(t *testing.T) *scenario {
	...
	api := simulator.NewAPI(logger, simdriver.NewDriver(engine, fixedClock{}))
...
func BenchmarkDisplayObservations(b *testing.B) {
	...
	logger := slog.New(slog.DiscardHandler)
	upstream := httptest.NewServer(simulator.NewAPI(logger, simdriver.NewDriver(engine, fixedClock{})))
