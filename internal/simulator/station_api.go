...
func (a *api) observations(w http.ResponseWriter, r *http.Request) {
	...
	o, err := a.sim.Observations(selection)
	if err != nil {
		a.stationError(w, r, err)
		return
	}
	a.writeJSON(w, r, observationsResponse(o))
}
...
func (a *api) receptions(w http.ResponseWriter, r *http.Request) {
	...
	page, err := a.sim.ReceptionHistory(values.Get("simulationId"), r.PathValue("id"), after, int(limit))
	if err != nil {
		a.stationError(w, r, err)
		return
	}
	a.writeJSON(w, r, receptionPageResponse(page))
}
