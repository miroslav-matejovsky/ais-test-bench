...
// serve runs the standalone simulator with the demonstration scenario on ln.
func serve(ctx context.Context, ln net.Listener, cfg config, logger *slog.Logger) error {
	return simulator.Serve(ctx, ln, simulator.StandaloneConfig{Simulation: simulator.DemoConfig(), Logger: logger, BasePath: cfg.basePath})
}
