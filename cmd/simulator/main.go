...
func run(args []string, logger *slog.Logger) error {
	...
	return simulator.Run(ctx, logger, ln)
}

// parseAddr parses the command-line arguments. The only flag is -addr, the
// HTTP listen address as host:port, validated by cli.ValidateListenAddr. It
// defaults to defaultAddr. Usage and flag errors are printed to output.
